...
class GeoPoint:
    ...
    def copy(self) -> "GeoPoint":
        """Create a copy of this GeoPoint instance.

        Returns:
            GeoPoint: A new GeoPoint instance with the same latitude and longitude.

        Example:
            >>> original = GeoPoint.from_deg(37.5665, 126.9780)
            >>> duplicate = original.copy()
            >>> print(duplicate)
            GeoPoint(latitude=37.5665 °N/S, longitude=126.978 °E/W)
        """
    ...
    def heading_to(self, other: "GeoPoint") -> Degree:
        """Calculate the initial heading (azimuth) to another GeoPoint.

        Uses the WGS84 ellipsoid model to compute the initial bearing angle
        from this point to the target point. The heading is measured in degrees
        clockwise from North.

        Args:
            other (GeoPoint): Target point to calculate heading to.

        Returns:
            Degree: Initial heading in degrees from North to the target point.
        """
        az12, az21, dist = _WGS84.inv(
            float(self.longitude),
            float(self.latitude),
